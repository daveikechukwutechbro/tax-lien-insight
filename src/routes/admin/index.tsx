...
function AdminOverview() {
  ...
  const stats: [string, number, string][] = [
    ...
    ["Total Bids", counts?.bids ?? 0, "/admin/bids"],
    ["Registered Users", counts?.users ?? 0, "/admin/users"],
  ];
      ...
      <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {stats.map(([label, val, href]) => (
          <Link
            key={label}
            to={href}
            className="rounded-xl border border-hairline bg-surface p-5 hover:border-navy"
          >

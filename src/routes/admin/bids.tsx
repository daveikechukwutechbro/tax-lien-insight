  ...
  const { data: rows = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("bids")
        .select(
          "id, placed_at, interest_rate, status, user:profiles(full_name), lien:liens(id, property:properties(address, city, state), auction:auctions(title))",
        )
  ...
  return (
    <div>
      ...
      <div className="mt-6 overflow-x-auto rounded-xl border border-hairline bg-surface">
        {rows.length === 0 ? (
          <p className="p-8 text-center text-sm text-ink-muted">No bids yet.</p>
        ) : (
            ...
            <tbody>
              {rows.map(
                (b: {
                  ...
                  const u = b.user;
                  const lien = b.lien;
                  const p = lien?.property;
                  const a = lien?.auction;
                  return (

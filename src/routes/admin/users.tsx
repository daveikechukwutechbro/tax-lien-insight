      ...
      <div className="flex items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-ink-muted">
            Manage roles, verification, balances, and messaging.
          </p>
        </div>
        <input
          value={q}
          ...
          className="h-9 w-64 rounded-md border border-hairline bg-surface px-3 text-sm"
        />
      </div>
                  ...
                  <tr key={p.id} className="border-b border-hairline/50 last:border-0">
                    <td className="px-4 py-2">
                      ...
                      <div className="text-xs text-ink-muted">
                        {p.id.slice(0, 8)} · {p.phone ?? "no phone"}
                      </div>
                    </td>
                    <td className="px-4 py-2 text-xs">{rs.length ? rs.join(", ") : "user"}</td>
                    <td className="px-4 py-2">
                      {p.verified ? (
                        <span className="rounded bg-success-soft px-2 py-0.5 text-xs font-600 text-success">
                          verified
                        </span>
                      ) : (
                    ...
                    <td className="px-4 py-2">
                      <div className="flex flex-wrap justify-end gap-1">
                        <button
                          ...
                          onClick={() => setRole(p.id, "admin", !isAdmin)}
                          className="rounded-md border border-hairline bg-surface p-1.5 hover:bg-surface-alt"
                        >
                          {isAdmin ? (

  ...
  return (
    <div>
      ...
      <div className="mt-4 overflow-x-auto rounded-xl border border-hairline bg-surface">
        {rows.length === 0 ? (
          <p className="p-8 text-center text-sm text-ink-muted">No submissions.</p>
        ) : (
                ...
                (r: {
                  ...
                  <tr key={r.id} className="border-b border-hairline/50 align-top last:border-0">
                    ...
                    <td className="px-4 py-2 font-500 text-navy">
                      {r.legal_name}
                      <div className="text-xs text-ink-muted">{r.profile?.full_name ?? ""}</div>
                    </td>
                    ...
                    <td className="px-4 py-2 text-right">
                      <div className="flex justify-end gap-2">
                        {r.id_document_path && (
                          <button
                            onClick={() => viewDoc(r.id_document_path)}
                        ...
                        {r.status === "pending" && (
                          <>
                            <button
                              ...
                              className="rounded-md bg-success px-2 py-1 text-xs font-600 text-white"
                            >
                              Approve
                            </button>
                            <button
                              onClick={() => review(r.id, false)}
                              className="rounded-md bg-destructive px-2 py-1 text-xs font-600 text-white"
                            >
                              Reject
                            </button>
                          </>

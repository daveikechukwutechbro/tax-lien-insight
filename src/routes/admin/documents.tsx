...
function DocumentsAdmin() {
  ...
  const { data: propsRows = [] } = useQuery({
    ...
    queryFn: async () =>
      (await supabase.from("properties").select("id, address, city, state").order("address"))
        .data ?? [],
  });
  ...
  return (
    <div>
      ...
      <p className="mt-1 text-sm text-ink-muted">
        Link property reports, tax records, deeds, and inspection files.
      </p>
      <form
        onSubmit={add}
      ...
      >
        ...
        <select
          ...
          className="input"
        >
          {["report", "tax_record", "deed", "inspection", "photo", "other"].map((k) => (
      ...
      <div className="mt-6 overflow-x-auto rounded-xl border border-hairline bg-surface">
        {rows.length === 0 ? (
          <p className="p-8 text-center text-sm text-ink-muted">No documents yet.</p>
        ) : (

  ...
  const { data: rows = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        .select(
          "id, created_at, user_id, auction:auctions(title, starts_at, status), user:profiles(full_name, verified)",
        )
  ...
  return (
    <div>
      ...
      <div className="mt-6 overflow-x-auto rounded-xl border border-hairline bg-surface">
        {rows.length === 0 ? (
          <p className="p-8 text-center text-sm text-ink-muted">No registrations yet.</p>
        ) : (
            ...
            <tbody>
              {rows.map(
                (r: {
                  ...
                  created_at: string;
                  user_id: string;
                  auction?: { title?: string; starts_at?: string; status?: string } | null;
                  user?: { full_name?: string; verified?: boolean } | null;
                }) => {
                  const u = r.user;
                  const a = r.auction;
                  return (
                    <tr key={r.id} className="border-b border-hairline/50 last:border-0">
                      ...
                      <td className="px-4 py-2">
                        {u?.full_name ?? r.user_id.slice(0, 8)}{" "}
                        {u?.verified && (
                          <span className="ml-1 rounded bg-success-soft px-1.5 text-[10px] font-600 text-success">
                            verified
                          </span>
                        )}
                      </td>

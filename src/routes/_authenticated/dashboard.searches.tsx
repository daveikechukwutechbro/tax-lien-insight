...
function Searches() {
  ...
  async function save(e: React.FormEvent) {
    e.preventDefault();
    if (!name.trim()) return toast.error("Give your search a name");
    try {
      await saveSearch({
        name,
        filters: { q: q || undefined, county: county || undefined, type: type || undefined },
        alertsEnabled: notify,
      });
  ...
  return (
    <div>
      ...
      <div className="mt-6 grid gap-3 sm:grid-cols-2">
        ...
        {rows.map((r) => {
          const query = r.filters as Record<string, string | undefined>;
          return (
